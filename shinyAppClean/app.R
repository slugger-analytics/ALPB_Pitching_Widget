...
#       player <- selected_pitcher()
#       if (is.null(player) || is.null(input$date_range)) {
#         file.create(file)  # empty placeholder PDF
#         return()
#       }
#       
...
ui <- fluidPage(
  fluidRow(
    ...
    column(10,
           ...
           fluidRow(
             column(4, card_w_header("Pitcher Info", uiOutput("player_info"))),
...
server <- function(input, output, session) {
  ...
  output$season_log <- renderTable({
    data.frame(W = sample(2:10, 1), L = sample(2:10, 1), ERA = round(runif(1, 3.00, 6.00), 2))
  })
