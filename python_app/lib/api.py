...
All external HTTP calls live here.  No caching — that's handled by cache.py.
"""
...
def fetch_all_pitchers(season_id: str = DEFAULT_SEASON_ID) -> pd.DataFrame:
    """Fetch all pitchers across every ALPB team for *season_id*."""
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as pool:
        futs = {pool.submit(_team_pitchers, t, season_id): t for t in teams}
...
def fetch_pitching_stats(
    ...
    """Fetch aggregated season stats for one player."""
...
def fetch_alpb_pitcher_info(fname: str, lname: str) -> dict | None:
    """Look up a pitcher's ALPB ID by name.  Returns dict or None."""
...
def fetch_alpb_pitches(player_id: str) -> pd.DataFrame | None:
    """Fetch all pitch-by-pitch data (paginated) for one pitcher."""
    if not player_id:
        return None
    ...
    if total_pages > 1:
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as pool:
            futs = {
                pool.submit(_alpb_page, url, player_id, p): p
...
def _alpb_page(url: str, player_id: str, page: int) -> list:
    try:
